...
func GetMedicineReceive(c *gin.Context) {
	var medicineReceive entity.MedicineReceive
	id := c.Param("id")
	if err := entity.DB().Preload("Pharmacist").Preload("Zone").Preload("MedicineLabel.Order").Raw("SELECT * FROM medicineReceives WHERE id = ?", id).Find(&medicineReceive).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func ListMedicineReceive(c *gin.Context) {
	var medicineReceive []entity.MedicineReceive
	if err := entity.DB().Preload("Pharmacist").Preload("Zone").Preload("MedicineLabel.Order").Raw("SELECT * FROM medicineReceives").Find(&medicineReceive).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	c.JSON(http.StatusOK, gin.H{"data": medicineReceive})
...
func DeleteMedicineReceive(c *gin.Context) {
	id := c.Param("id")
	if tx := entity.DB().Exec("DELETE FROM medicineReceives WHERE id = ?", id); tx.RowsAffected == 0 {
